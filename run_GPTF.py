...
from baselines.GPTF_linear import *
from baselines.GPTF_rnn import *
from baselines.GPTF_time import *
...
def evaluation(**kwargs):
    ...
    if config.trans=='linear':
        ...
        raise Exception('Error in run_GPTF.py')
    #
...
#     cprint('g', nmod)
    ...
    if config.trans=='linear':
        ...
        model = GPTF_time(train_ind, train_y, train_time, U, m, batch_size, torch.device('cpu'))
    else:
        raise Exception('Error in run_GPTF.py')
    #
